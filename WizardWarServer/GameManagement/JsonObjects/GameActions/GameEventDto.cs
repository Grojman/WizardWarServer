...
public record GameEventDto(Guid Source, Guid PlayerSource)
{
    public static GameEventDto Generate(GameEvent e, GameState state)
    {
        return e switch
        {
            ...
            GameEvent.UnitPlayed up => new UnitPlayed(up.Source.Id, up.PlayerSource.Id, CardDto.Generate(up.Unit, state, true), up.BoardPosition),
            GameEvent.SpellPlayed sp => new SpellPlayed(sp.Source.Id, sp.PlayerSource.Id, CardDto.Generate(sp.Spell, state, true)),
